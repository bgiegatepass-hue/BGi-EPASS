  ...
  async _openWebCameraScanner() {
    ...
    const startCamera = async () => {
      ...
      try {
        ...
        // Prefer environment (back) camera when available. If deviceId is not available
        // (labels hidden until permission), fall back to facingMode and try.
        let constraints = {
          video: {
        ...
        // Try to pick a specific rear camera device if enumerateDevices provides labels
        try {
          const devices = await navigator.mediaDevices.enumerateDevices();
          const videoInputs = devices.filter(d => d.kind === 'videoinput');
          const preferred = videoInputs.find(d => /back|rear|environment|camera/i.test(d.label));
          if (preferred && preferred.deviceId) {
            constraints.video.deviceId = { exact: preferred.deviceId };
          }
        } catch (e) {
          // ignore enumerateDevices failures; continue with facingMode
        }

        try {
          stream = await navigator.mediaDevices.getUserMedia(constraints);
        } catch (err) {
          // Retry without facingMode/deviceId if the first attempt fails
          try {
        ...
        if (name === 'NotAllowedError' || name === 'PermissionDeniedError') {
          status.textContent = '❌ Permission denied. Allow camera access in browser or app settings.';
        } else if (name === 'NotFoundError' || name === 'OverconstrainedError') {
          status.textContent = '❌ No camera found or requested resolution not supported.';
        } else if (e.message && e.message.indexOf('secure') !== -1) {
          status.textContent = '❌ Camera requires secure context (HTTPS or localhost).';
        } else {
