    ...
    try {
      ...
      body.innerHTML = `
        ...
        <div style="display:flex;gap:12px;margin-top:16px;">
          ...
          ${(epass.pdf_url || epass.pdfUrl) ? `<ion-button style="flex:1.4;" id="epass-download-btn">
      ...
      document.getElementById('epass-share-btn').addEventListener('click', async () => {
        const shareText = epass.pdf_url || epass.pdfUrl
          ? `Approved E-Pass: ${epass.pass_id || epass.passId}. Download: ${epass.pdf_url || epass.pdfUrl}`
          : `Approved E-Pass ID: ${epass.pass_id || epass.passId}`;
        ...
        if (window.Capacitor?.Plugins?.Share) {
          ...
          await navigator.share({ title: 'My E-Pass', text: shareText });
        } else {
          await navigator.clipboard.writeText(shareText);
          await UI.toast('Pass details copied to clipboard');
        }
      });
    } catch (e) {
